...
// ─── Блок важных периодов с выпадашкой ───────────────────────────────────────
function AlertsBlock({ grouped, fmtDate, openClientTransits }) {
        ...
        {grouped.map(g => (
          <div key={g.client_id} style={{ borderRadius: 8, background: 'rgba(139,92,246,0.06)', overflow: 'hidden' }}>
            ...
            >
              ...
              <span style={{ ...S.muted, fontSize: 16 }}>{openIds[g.client_id] ? '▲' : '▼'}</span>
            </div>
            {openIds[g.client_id] && g.events.map((a, j) => (
              <div
                key={j}
              ...
              >
                ...
                <span style={{ ...S.muted, whiteSpace: 'nowrap' }}>
                  {fmtDate(a.exact_date)}{a.orb != null ? ` · орб ${a.orb}°` : ''}
                </span>
              </div>
            ))}
          </div>
