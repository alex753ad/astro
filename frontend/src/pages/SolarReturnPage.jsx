/**
 * Соляр — карта солнечного возвращения на выбранный год.
 * Route: /solar-return/:chartId. Доступ только администраторам.
 */
...
import { calculateSolarReturn, streamSolarReturnInterpretation } from '../api/client';
...
function PlaceInput({ value, onChange, placeholder }) {
...
export default function SolarReturnPage() {
  ...
  async function handleCalculate() {
    ...
    setSolar(null);
    try {
      setSolar(await calculateSolarReturn(chartId, year, location || null));
    } catch (e) {
      setError(e.message);
    } finally {
      setLoading(false);
  ...
  const exactMoment = solar?.solar_return_datetime
    ? new Date(solar.solar_return_datetime).toLocaleString('ru-RU', {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8">
      ...
      <p style={{ fontSize: 14, color: 'var(--text-secondary)', marginBottom: 24 }}>
        Карта на момент возврата Солнца в натальную позицию — темы года от дня
        рождения до следующего.
      </p>
      ...
      <Field label="Город" hint="По умолчанию — место рождения">
        <PlaceInput
          value={location}
          onChange={setLocation}
          placeholder="по умолчанию — место рождения"
        />
      </Field>

      <MotionButton onClick={handleCalculate} disabled={loading} style={{ width: '100%' }}>
        {loading ? 'Считаем…' : 'Рассчитать соляр'}
      </MotionButton>
        ...
        <div style={{ marginTop: 28 }}>
          {exactMoment && (
            <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 14 }}>
              Точный момент возврата Солнца: <strong>{exactMoment}</strong> (UTC)
            </p>
          )}
          ...
          <NatalChart
            ...
            houses={solar.houses}
            aspects={solar.aspects}
            ascendant={solar.ascendant}
            midheaven={solar.midheaven}
            timeUnknown={solar.time_unknown}
          />

          <div style={{ marginTop: 24 }}>
            <AspectTable aspects={solar.aspects} planets={solar.planets} />
          </div>

          <AdvancedInterpretation
            buttonLabel="Получить разбор года"
            start={(onChunk, onDone, onError) =>
              streamSolarReturnInterpretation(
                chartId, year, location || null, onChunk, onDone, onError,
              )
            }
          />
        </div>
