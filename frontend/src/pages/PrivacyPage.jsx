    ...
    <div style={s.page}>
      <div style={s.inner}>
        ...
        {[
          ...
          { label: 'Оплата', text: 'Мы не храним данные банковских карт. Платёжную информацию обрабатывает ЮKassa. Мы получаем только статус транзакции и идентификатор подписки.' },
        ...
        <p style={s.p}>— Генерация AI-интерпретаций (передаются только астрономические параметры карты, без имени, email и иных контактных данных).</p>
        <p style={s.p}>— Отправка персонализированных писем (прогнозы, дайджесты). Вы можете отписаться в любой момент.</p>
        <p style={s.p}>— Обработка платежей и управление подпиской через ЮKassa.</p>
        ...
        <p style={s.p}>
          Данные хранятся на серверах ООО «ТАЙМВЭБ.КЛАУД» на территории Российской Федерации.
        ...
        {[
          { label: 'ЮKassa', text: 'Обработка платежей. Данные карт хранятся исключительно у платёжного сервиса.' },
          { label: 'AI-интерпретации', text: 'Сервисы генерации AI-интерпретаций — передаются только астрономические параметры карты, без имени, email и иных контактных данных. Обработка производится на серверах за пределами РФ.' },
          { label: 'Email-уведомления', text: 'Сервис отправки email-уведомлений.' },
          { label: 'Google LLC', text: 'Интеграция с Google Calendar через OAuth 2.0 — только при наличии явного согласия пользователя, исключительно для функций планера. Политика Google: policies.google.com/privacy' },
        ].map(({ label, text }) => (
          <div key={label} style={s.card}>
            <div style={s.label}>{label}</div>
            <p style={{ ...s.p, margin: 0 }}>{text}</p>
        ...
        <h2 style={s.h2}>8. Характер услуги</h2>
        <p style={s.p}>
          Астрология не является наукой. Расчёты и интерпретации на сайте носят информационный
          и развлекательный характер. Сервис не предоставляет медицинских, финансовых, юридических
          или психологических консультаций — решения на основе материалов сайта пользователь
          принимает самостоятельно. Подробнее — в <Link to="/terms" style={s.link}>оферте</Link>.
        </p>

        <h2 style={s.h2}>9. Изменения политики</h2>
        <p style={s.p}>
