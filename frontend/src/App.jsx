    ...
    <header className="sticky top-0 z-50 bg-brand-card/80 backdrop-blur-md border-b border-brand-border">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
        ...
        {/* Логотип + слоган — одна строка с навигацией, решение владельца 17.09.2026.
            ...
            была настоящая: слоган стоял в одном ряду с меню и НАЕЗЖАЛ на его
            пункты. Возврат в одну строку не отменяет ту причину, а снимает её
            иначе.

            ⚠️ Слоган НЕ ОБРЕЗАЕТСЯ. Первый заход 17.09.2026 поставил `truncate`,
            и на промежуточных ширинах фраза вставала как «…по ритму космиче…» —
            обрывок на полуслове читается как поломка вёрстки. Сокращать тут
            нечего: у слогана нет короткой формы. Поэтому он показывается
            ЦЕЛИКОМ или не показывается вовсе (`whitespace-nowrap` + `shrink-0`:
            сжаться и попасть под `overflow-hidden` он не может в принципе).

            ⚠️ Показывается он только РАЗЛОГИНЕННОМУ, и это не про вкус, а про
            арифметику. Контейнер шапки ограничен `max-w-6xl`, то есть 1120px
            содержимого НА ЛЮБОЙ ширине экрана — шире он не становится никогда.
            Замер 17.09.2026 в боевых шрифтах: слоган 448px, бренд 153px, самое
            длинное меню (премиум с картой: карта, планер, календарь, «Скоро»,
            кабинет астролога, имя, «Выйти», тумблер) — 826px плюс промежутки.
            Вместе 1563 при доступных 1120: не помещается, и никакой брейкпоинт
            этого не меняет, потому что решает не ширина экрана, а длина меню.
            Без слогана то же меню занимает 1115 — впритык, но помещается.

            У разлогиненного меню это «Войти» и тумблер, около 114px, и слоган
            с ним уживается от 1024px — отсюда `lg`. Ниже он скрыт.

            Практическое следствие: добавляя пункт меню, не удивляться, что
            слоган пропал у части людей, — он и так виден только гостям, а
            запас у меню всего 5px. */}
        <Link to="/" className="flex items-center gap-2 group h-8 overflow-hidden min-w-0">
          ...
          {!user && (
            <span className="hidden lg:block shrink-0 text-sm text-brand-muted border-l border-brand-border pl-3 ml-1 whitespace-nowrap">
