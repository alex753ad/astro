...
export default function FeedTimelineNode({
  ...
  quiet = false, solid = false, pad = 16,
}) {
  ...
  return (
    <div style={{ position: 'relative', paddingBottom: gap }}>
      ...
      <div style={{ display: 'flex', minHeight: (hasDot && !dense) ? 44 : undefined }}>
        ...
        {/* ⚠️ Зазор между линией и содержимым РАЗНЫЙ, и это не небрежность.
            16.09.2026 его свели к 8 у всех узлов разом, чтобы отдать ширину
            расшифровке периода. Карточке периода это помогло, а транзиту
            стоило дефекта: точка на линии — кольцо радиусом до 6.5px от оси
            (ось на 47, содержимое начиналось с 56), и первый значок формулы
            («○☽», «○☿») вставал к ней вплотную — приёмка 17.09.2026.

            Поэтому умолчание вернулось к 16, а восемь остались ТОЛЬКО у
            карточки периода, которую и просили не трогать: она рисует рамку,
            а не значок, и вплотную к точке не подходит. Значение приходит
            параметром от вызывающего (FeedScreen.jsx), а не вычисляется здесь
            по виду события: узел про вид ничего не знает и знать не должен. */}
