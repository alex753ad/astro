  ...
  return (
    ...
    >
      ...
      <div style={{ fontSize: 13.5, lineHeight: 1.5, color: 'var(--text-secondary)' }}>
        ...
        одним уведомлением в день, начиная с выбранного тобой времени. Что именно приходит, настраивается
...
export default function MoreDeviceChannel() {
  ...
  return (
    <>
      ...
      {on && channel === 'device' ? (
        <p style={note}>
          На этом телефоне уведомления работают в упрощённом режиме: они появятся, когда ты
          возьмёшь телефон в руки, а не в тот же момент. Так бывает на устройствах без
          сервисов Google.
        </p>
      ) : null}
      ...
      {problem === 'none' && !intro ? (
        <p style={note}>
          Не удалось включить уведомления на этом устройстве. Проверь, что уведомления
