 ...
 *
 * ⚠️ Текущие лимиты тарифа здесь НЕ показываются числом — ни разу.
 * `features.transits_months` из /profile/subscription для free равен 0,
 * хотя фактический горизонт транзитов — 3 месяца (FREE_TRANSITS_TEASER_MONTHS
 * на бэкенде, мимо этого флага). Число из живых `features`/`limits` в этот
 * блок не подставлять никогда — только статический текст `tierFeatures()`
 * (SPEC_MORE_SCREEN.md §4.2).
 *
 * ⚠️ Пункт «Транзиты: горизонт N месяцев…» из списка «На … дополнительно»
 * ИСКЛЮЧЁН намеренно, не по ошибке. Число там сегодня не врёт — совпадает
 * с TIER_FLAGS соответствующего тарифа. Но горизонт транзитов живёт тремя
 * независимыми копиями (сам флаг, FREE_TRANSITS_TEASER_MONTHS-исключение
 * для free, и этот статический текст в constants.js), и на free они уже
