...
/**
 * Показывать ли «ответ не дошёл» — решается ПОСЛЕ того, как поток завершился.
 *
 * ⚠️ **Здесь до 09.09.2026 стояло `shouldReportInterrupted({visible, streaming})`,
 * и оно было неверно по построению.** Решение принималось в обработчике
 * возврата из фона по флагу `streaming`, а тот означает «промис ещё не
 * разрешился», а НЕ «текста нет». На возврате из фона он гарантированно
 * `true`: заморозить WebView можно ровно тогда, когда запрос в полёте. Значит
 * сообщение показывалось ВСЕГДА — в том числе поверх полностью пришедшего
 * ответа.
 *
 * Замерено прогоном настоящего `streamChatAnswer`:
 *
 *     текст на момент возврата : "Ваш Марс в пятом доме."  ← ответ уже целиком
 *     streaming на возврате    : true
 *     показано «не дошёл»      : true                       ← ложь
 *
 * Признак завершения при этом нигде не терялся: `[DONE]` разбирался верно, но
 * ПОЗЖЕ обработчика. Поэтому лечение — не «уточнить условие», а перенести само
 * решение туда, где оно может быть верным: на момент, когда поток кончился.
 *
 * ⚠️ Отсюда же следует, почему обработчика возврата из фона больше нет вовсе.
 * Любая проверка в этот момент гонится с обработкой уже пришедших байтов: даже
 * «пришёл ли текст» на возврате даст `false`, если чанки ещё не разобраны.
 * Момент возврата в принципе не годится как точка решения.
 *
 * Поток по-прежнему НЕ перезапрашивается (решение владельца) — это отличие от
 * натального разбора, где `shouldRefetchOnResume` перезапускает: там разбор
 ...
 *
 * @param {{streaming: boolean, gotText: boolean, failed: boolean}} state
 * @returns {boolean}
 */
export function shouldReportNoAnswer({ streaming, gotText, failed }) {
  if (streaming) return false;   // поток ещё жив — судить рано
  if (failed) return false;      // отказ уже показан своим, более точным текстом
  return !gotText;
}
