 ...
 *
 * ⚠️ Дата начала здесь НЕ повторяется, и это не экономия места, а правило
 * ленты: колонку слева от строки занимает `dateShort(at)` — тот же узел линии
 * (FeedTimelineNode), что у всех периодов. Написать дату ещё раз значило бы
 * показать одно и то же число дважды в одной строке.
 *
...
export function moonRangeShort(fromAt, toAt) {
  ...
  return `${timePart(fromAt)} — ${dateShort(toAt)} ${timePart(toAt)}`;
}
