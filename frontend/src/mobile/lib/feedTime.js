 ...
 * поясе, который сервер назвал в поле `timezone` ответа. С 24.09.2026 это
 * пояс ТЕЛЕФОНА (feedApi шлёт `tz`), пояс карты — только если его не
 * прислали. Клиент всё равно не пересчитывает эти моменты: пояс ответа и
 * пояс устройства могут разойтись (ответ из офлайн-кэша, снятый до
 * перелёта), и `new Date(at).getHours()` вернул бы час, которого нет в
 * остальной ленте. Поэтому дата и время из `at` берутся
