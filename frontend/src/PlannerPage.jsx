...
function usePlannerExport() {
  ...
  function getToken() {
    return new Promise((resolve, reject) => {
      ...
      if (!clientId) return reject(new Error('VITE_GOOGLE_CLIENT_ID не задан'));
      ...
      const popup = window.open(`https://accounts.google.com/o/oauth2/v2/auth?${params}`, 'gcal_oauth', 'width=500,height=620,left=200,top=100');
      const timer = setInterval(() => {
        try {
          ...
          if (url.includes('access_token')) {
            ...
            tokenRef.current = hash.get('access_token');
            resolve(tokenRef.current);
          }
          if (popup?.closed && !url.includes('access_token')) { clearInterval(timer); reject(new Error('Отменено')); }
        } catch (_) {}
      }, 300);
    });
  }

  async function exportPlan(planData, monthTitle) {
    if (!planData) return;
    setStatus('loading');
    try {
      ...
      (planData.month_sections || []).forEach(section => {
        (section.periods || []).forEach(p => {
          if (p.period) {
            const match = p.period.match(/(\d{2}\.\d{2})\s*[-–]\s*(\d{2}\.\d{2})/);
            if (match) {
              const [d1, m1] = match[1].split('.');
              const [d2, m2] = match[2].split('.');
              events.push({
                summary: `🪐 ${section.planet_title || section.title} — ${(p.spheres || []).join(', ')}`,
...
export default function PlannerPage() {
  ...
  const { exportPlan, status: gcalStatus } = usePlannerExport();
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "24px 16px" }}>
        ...
        {!isFree && (loading ? (
          ...
          <>
            ...
            {tab === "month" && (
              <button
                onClick={() => exportPlan(planData, planData?.month_title)}
                disabled={gcalStatus === 'loading'}
                style={{
                  width: '100%', marginBottom: 16, padding: '10px 16px',
                  borderRadius: 10, border: '1px solid #334155',
                  background: gcalStatus === 'success' ? '#14532d' : gcalStatus === 'error' ? '#7f1d1d' : '#1e293b',
                  color: gcalStatus === 'success' ? '#86efac' : gcalStatus === 'error' ? '#fca5a5' : '#e2e8f0',
