 ...
 *
 * ⚠️ **Единственным местом это станет только после того, как useOGMeta в
 * App.jsx начнёт читать отсюда же.** Сегодня у неё свои литералы на три
 * группы маршрутов, и они ПЕРЕЗАПИСЫВАЮТ пререндеренные значения, как только
 * выполняется JS. Замерено 14.09.2026 сравнением документа без JS и после:
 *
 *   /            title и og:* — расходятся (регистр в заголовке, другой текст
 *                описания); description и canonical — совпадают
 *   /zodiac/leo  то же самое
 *   /pricing     совпадает всё: ветки для этого маршрута в useOGMeta нет
 *   /terms       то же
 *
 * Содержимое страниц при этом идентично, h1 совпадает везде — расходятся
 * только метаданные. Google исполняет JS, поэтому для «/» и «/zodiac/*» в
 * индекс попадает вариант из App.jsx, а не отсюда. Оба текста осмысленные,
 * поэтому сегодня это не вред, а расхождение: объявленное здесь не является
 * тем, что реально видит поисковик. Чинится переводом useOGMeta на этот файл
 * (правка App.jsx, на 14.09.2026 не согласована).
 */
