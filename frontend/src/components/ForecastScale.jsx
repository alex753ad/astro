...
export default function ForecastScale({ chartId, selectedDate: externalDate }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      let url;
      if (scale === 'day') {
        url = `${API_BASE}/chart/${chartId}/forecast/daily?on_date=${selectedDate}`;
      } else if (scale === 'week') {
        const monday = getMonday(selectedDate);
        const d      = new Date(monday + 'T00:00:00');
        d.setDate(d.getDate() + 6);
        const sunday = d.toISOString().slice(0, 10);
        url = `${API_BASE}/chart/${chartId}/forecast/weekly?week_start=${monday}&week_end=${sunday}`;
      } else {
        const d     = new Date(selectedDate + 'T00:00:00');
        const y     = d.getFullYear();
        const m     = String(d.getMonth() + 1).padStart(2, '0');
        const last  = new Date(y, d.getMonth() + 1, 0).getDate();
        url = `${API_BASE}/chart/${chartId}/forecast/monthly?from_date=${y}-${m}-01&to_date=${y}-${m}-${last}`;
      }
