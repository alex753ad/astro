...
const MOCK_INTERPRETATIONS = {
  "Jupiter conjunction Sun": "Транзит Юпитера в соединении с твоим натальным Солнцем — один из самых благоприятных транзитов, который случается раз в 12 лет.\n\nЭто период расширения возможностей, когда твоя уверенность в себе растёт, а жизнь словно открывает перед тобой новые двери. Юпитер усиливает всё, к чему прикасается, и в данном случае он усиливает твою жизненную силу, самовыражение и чувство цели.\n\nОбрати внимание на возможности в сфере карьеры и личного развития. Это не время скромничать — Юпитер вознаграждает смелость и оптимизм.\n\nРекомендация: используй этот период для запуска проектов, которые давно откладывались. Энергия благоприятствует масштабному мышлению.",
  "Pluto conjunction Saturn": "Транзит Плутона в соединении с твоим натальным Сатурном — глубокий и трансформирующий процесс, который затрагивает сами основы твоей жизненной структуры.\n\nСатурн отвечает за правила, границы, дисциплину и то, как ты организуешь свою жизнь. Плутон приходит и проверяет: насколько эти структуры подлинны?\n\nЭтот транзит может ощущаться как давление со стороны обстоятельств: карьерные изменения, перестройка жизненных приоритетов. Но его цель — не разрушение, а обновление.\n\nТо, что выдержит этот транзит, станет несокрушимым фундаментом для следующего этапа твоей жизни.",
  "Uranus square Mercury": "Транзит Урана в квадрате к твоему натальному Меркурию вносит электрическое напряжение в сферу мышления и коммуникации.\n\nТы можешь обнаружить, что привычные способы думать и общаться перестают работать. Неожиданные идеи, внезапные озарения, но также нервозность и рассеянность — всё это проявления этого аспекта.\n\nРекомендация: записывай идеи, но не торопись с их реализацией. Перепроверяй важные документы и договоры.",
};
      ...
      <div>
        <button onClick={() => toggle("planets")} style={filterLabelStyle}>
          ...
          {planetFilter.length > 0 && planetFilter.length < ALL_PLANETS.length && (
            <span style={{ background: "var(--accent-glow)", color: "var(--bg-card)", borderRadius: 'var(--radius-md)', padding: "1px 7px", fontSize: 11, fontWeight: 700 }}>{planetFilter.length}</span>
          )}
        </button>
...
function FreePlanBanner({ lockedCount, featuredTransit, onUpgrade }) {
  ...
  return (
    <div style={{
      margin: "8px 0", padding: "16px 20px", borderRadius: 'var(--radius-lg)',
    ...
    <div
      ...
      style={{
        ...
        background: isSelected
          ...
          : (hovered ? "rgba(var(--accent-rgb), 0.06)" : "transparent"),
  ...
  return (
    <div className="tt-scope" style={{ fontFamily: "var(--font-body)", maxWidth: 900, margin: "0 auto", padding: "24px 16px", color: "var(--text-primary)" }}>
      <style>{`
        ...
        .tt-event-card:hover { transform: translateY(-2px); box-shadow: var(--shadow-raised); }
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
          {loading ? (
            ...
            <div style={{ padding: 40, textAlign: "center", color: "var(--color-danger)", fontSize: 14, borderRadius: 'var(--radius-lg)', border: "1.5px dashed var(--accent-hairline)", background: "var(--bg)" }}>
              ...
              <span style={{ fontSize: 12, opacity: 0.7 }}>Попробуй обновить страницу.</span>
            </div>
            ...
            <div style={{ padding: 40, textAlign: "center", color: "var(--text-secondary)", fontSize: 14, borderRadius: 'var(--radius-lg)', border: "1.5px dashed var(--accent-hairline)", background: "var(--bg)" }}>
              ...
              <span style={{ fontSize: 12, opacity: 0.7 }}>Попробуй увеличить орб или сбросить фильтры.</span>
            </div>
          ) : (
