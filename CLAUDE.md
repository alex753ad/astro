...
(`frontend/src/constants.js`); оба обязаны совпадать. `charts_per_month` и
`profiles_limit` синхронизированы на одно число по каждому тарифу (раньше
расходились — `charts_per_month` был месячной скоростью создания карт,
`profiles_limit` показывался на витрине, но не проверялся технически; с
19.08.2026 оба поля равны и оба enforced). Лимиты применяются через
`TierRateLimiter` (`auth/rate_limits.py`), расход считается в `UsageCounter`.
Монотонность сетки (платный тариф не хуже дешёвого ни по одному полю)
проверяется тестом `TestTierMonotonicity` в `test_rate_limits.py`.
