...
**TODO, обязательно до начала работ по подключению ЮKassa** (отдельным
коммитом, не смешивать с платежами): переделать самокопирование в
`05-update.sh` на temp-файл + atomic `mv`. Причина срочности именно к
этому сроку — прод-гварды, снятые 19.08.2026 (см. ниже), вернутся под
переменные ЮKassa в этот же файл, то есть его размер снова изменится и
снова попадёт в ту же рулетку, если фикс не сделан заранее.
