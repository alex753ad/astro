...
class RefreshRequest(BaseModel):
    # Необязателен: в вебе основной источник — HttpOnly-кука astro_refresh.
    #
    # ⚠️ Если убрать это поле: мобильное приложение (Capacitor) потеряет
    # возможность обновлять access-токен и отзывать refresh при выходе —
    # пользователей начнёт выбрасывать из аккаунта примерно через час. Веб-тесты
    # при этом останутся полностью зелёными: в вебе поле не используется.
    # Куку webview получить не может, см. MOBILE_CLIENT_HEADER в auth/router.py.
...
class TokenResponse(BaseModel):
    access_token: str
    # Пусто для веба: refresh уезжает HttpOnly-кукой astro_refresh, куда не
    # дотянется ни XSS, ни скомпрометированная npm-зависимость.
    #
    # Непустым приходит в двух случаях (см. _build_token_response): запрос
    # пришёл от мобильного клиента с заголовком X-Client-Platform, либо старая
    # сборка фронта прислала refresh в теле запроса.
    #
    # ⚠️ Если убрать это поле: мобильному приложению будет неоткуда взять
    # refresh — куку webview не получает, — и пользователей начнёт выбрасывать
    # из аккаунта примерно через час. В вебе ничего не сломается, и тесты
    # останутся зелёными.
    refresh_token: Optional[str] = None
