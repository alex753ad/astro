...
class InterpretationRequest:
    ...
    # ── Обратная связь от роутера (заполняется им, не вызывающей стороной) ──
    # Какой движок реально отдал текст. Нужно тем, кто сохраняет результат
    # стрима: у InterpretationResult поле engine есть, а stream() отдаёт
    # голые строки и иначе движок не назвать.
    #
    # Поле лежит здесь, а не на InterpretationRouter, СПЕЦИАЛЬНО: get_router()
    # возвращает синглтон, атрибут на нём был бы общим для всех одновременных
    # запросов, и под нагрузкой разбор одного пользователя записался бы с
    # движком другого. Объект запроса свой на каждый вызов — гонки нет.
    engine_used: str | None = None
    # Чем кончился стрим, глазами провайдера: "stop" — модель дописала сама.
    # Ставится в InterpretationRouter.stream() тем же приёмом, что engine_used
    # выше: генератор отдаёт только текст, а вызывающей стороне (SSE-эндпоинт)
    # нужен ещё и факт завершения, чтобы не выводить его из наличия маркера.
