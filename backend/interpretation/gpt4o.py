...
# ~2.1 ток/слово на выходе — близко к оценке.
#
# ⚠️ Перевыполнение объёма, ради которого здесь стоял АДДИТИВНЫЙ запас в 1500
# токенов, было не свойством модели, а дефектом промпта: рядом с числом слов
# стояло независимое число абзацев, и модель слушала абзацы (разбор — в
# _volume_plan, interpretation/prompts.py). 09.09.2026 причина устранена,
# и запас переведён на МНОЖИТЕЛЬ.
#
# Почему форма важнее величины: плоское слагаемое даёт тарифам РАЗНЫЙ
# относительный запас — тем меньший, чем крупнее тариф. До правки:
#   free    500 слов → 2750 токенов, запас 2.20× к цели
#   lite    800      → 3500,         1.75×
#   pro    2500      → 7750,         1.24×
#   premium 5000     → 14000,        1.12×
# То есть вероятнее всех упирался в потолок premium — тариф, которому в том
# же промпте говорили «НЕ МЕНЕЕ 5000 слов». А упереться здесь значит получить
# finish_reason=length → IncompleteInterpretation → ОТКАЗ вместо разбора
# (router.py), причём у самого дорогого тарифа.
#
# Множитель выравнивает запас: у всех тарифов одинаковые 1.6 к оценке
# «слова × 2.5». В пересчёте на реальные ~2.1 ток/слово это значит, что любой
# тариф может честно перевыполнить объём примерно в 1.9 раза и всё равно
# остановиться сам, а не быть обрезанным.
_TOKENS_PER_WORD = 2.5
_MAX_TOKENS_HEADROOM = 1.6
...
def _calc_max_tokens(request) -> int:
    ...
    Запас — множитель, а не слагаемое: см. разбор выше. Потолок здесь только
    страхует от разгона, целевой объём задаёт промпт."""
    words = resolve_word_limit(request)
    return int(words * _TOKENS_PER_WORD * _MAX_TOKENS_HEADROOM)
