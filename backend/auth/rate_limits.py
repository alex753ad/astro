...
#
# 31.08.2026: раньше витрина (12 мес) была БОЛЬШЕ transits_months Лиры
# (3 мес) — платный тариф оказывался хуже бесплатного по видимому горизонту.
# Решение владельца: витрина сокращена до 3, и сетка Вега/Лира/Орион
...
# молча сломала бы одну из них. Под блюром должно быть что показать, иначе
# FreePlanBanner со счётчиком закрытых транзитов и PlanComparisonModal
# остаются без данных, и апселл не на чем строить — НЕ «чинить» обнулением
# или приравниванием к transits_months.
#
# ⚠️ Число обязано совпадать с литералом 3 в `TransitTimeline.jsx` (строка с
# `const maxMonths = isFree ? 3 : ...`) — там оно НЕ выводится из флага, это
