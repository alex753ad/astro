...
# ── Склонение знаков зодиака ─────────────────────────────────────────────────
#
# ⚠️ Нужны ДВА падежа, и это не избыточность: «Новолуние в Деве» — предложный
# («в» где?), «Меркурий входит в Деву» — винительный («в» куда?). Один и тот же
# предлог, разные вопросы. До 16.09.2026 обе фразы строились подстановкой
# именительного и читались как «в Дева».
#
# ⚠️ Словарь перечислением, а не правилом по окончанию: «Весы» и «Рыбы» стоят
# во множественном («в Весах», «в Весы»), «Лев» теряет беглую «е» («во Льве»),
# «Козерог» и «Скорпион» склоняются как обычные мужские. Суффиксного правила,
# покрывающего это, нет — ровно та же причина записана у ruDeclension.js на
# фронте.
#
# ⚠️ Второе место склонений на бэкенде (первое — `_plural` в письмах,
# числительные). Сводить их не с чем: разные категории слов. На фронте свой
# словарь знаков (ruDeclension.js) — он нужен там, где строку собирает клиент
# (полоса «сейчас» в ленте); здесь — там, где её собирает сервер.

# ⚠️ ПРЕДЛОГ входит в значение, а не подставляется рядом. У Льва он «во», а не
# «в» («во Льве», «во Льва»): перед стечением согласных «Льв» русский требует
# беглой гласной. Оставь предлог в шаблоне — и одна строка из двенадцати будет
# читаться неграмотно, причём заметит это только тот, кто попадёт на Льва.

_SIGN_PREPOSITIONAL = {
    "Овен": "в Овне", "Телец": "в Тельце", "Близнецы": "в Близнецах",
    "Рак": "в Раке", "Лев": "во Льве", "Дева": "в Деве", "Весы": "в Весах",
    "Скорпион": "в Скорпионе", "Стрелец": "в Стрельце", "Козерог": "в Козероге",
    "Водолей": "в Водолее", "Рыбы": "в Рыбах",
}

_SIGN_ACCUSATIVE = {
    "Овен": "в Овна", "Телец": "в Тельца", "Близнецы": "в Близнецов",
    "Рак": "в Рака", "Лев": "во Льва", "Дева": "в Деву", "Весы": "в Весы",
    "Скорпион": "в Скорпиона", "Стрелец": "в Стрельца", "Козерог": "в Козерога",
    "Водолей": "в Водолея", "Рыбы": "в Рыбы",
}


def sign_in(sign: str) -> str:
    """«в Деве», «во Льве» — вместе с предлогом. Незнакомое — «в {как пришло}»."""
    return _SIGN_PREPOSITIONAL.get(sign, f"в {sign}")


def sign_into(sign: str) -> str:
    """«в Деву», «во Льва» — вместе с предлогом. Незнакомое — «в {как пришло}»."""
    return _SIGN_ACCUSATIVE.get(sign, f"в {sign}")
...
def _find_phase(jd_start: float, jd_end: float, target: float) -> list[float]:
    ...
    останавливается на последней точке строго меньше границы. Смена знака
    в последнем интервале (jd_end − step, jd_end] поэтому не с чем
    сравнивать: `val` в правом конце интервала попросту не существует.
    Фаза, попавшая в последние 12 часов запрошенного окна, не находится
    вовсе — не текущим вызовом и не следующим (у get_moon_phases это
    следующий календарный месяц, а его собственное сканирование начинается
    РОВНО с jd_end и назад не заглядывает).

    Найдено сверкой с независимым расчётом на 10 годах: 5 потерянных фаз,
    первая — новолуние 31.08.2027 17:41 UTC. Симптом мог не поймать даже
    внимательный взгляд на код: `_find_phase` выглядит корректной, потому
    что условие `jd < jd_end` — обычный и уместный способ ограничить
    полуоткрытый интервал; ошибка не в самом условии, а в том, что
    ПОСЛЕДНЯЯ проверяемая пара (prev, val) требует значения val ИМЕННО в
    jd_end, а его взять неоткуда без этой лишней итерации.

    Лечится расширением условия на один шаг. Структурно `found` из-за этого
    не может оказаться позже jd_end: бисекция ищет корень внутри [lo, hi] ⊆
    [jd_end − step, jd_end], и `hi` не превышает jd_end ни на одной
    итерации. Это следует из кода, но раньше нигде не проверялось явно —
    гарантией служил только докстринг «оба вызывающих передают границы,
    кратные шагу». Третий вызывающий с произвольным (не кратным шагу) окном
    сломал бы это молча, поэтому ниже стоит явный отсев `found > jd_end` —
    он не должен срабатывать никогда при текущих вызывающих, но if будущий
    вызывающий его всё же зацепит, лишний момент отбросится, а не утечёт
    в чужой месяц.

    ⚠️ Отдельный, гораздо более узкий случай — фаза, чей истинный момент
    приходится практически ТОЧНО на jd_end (полночь первого числа
    следующего месяца). Проверено исполнением на 248 фазах за 2026–2035:
    ни одна не легла на 00:00 первого числа. Не отсеивается по календарному
    месяцу и не документируется отдельным кодом намеренно: 48 итераций
    бисекции на 12-часовой скобке дают точность на много порядков точнее
    секунды, поэтому `found` совпадёт с jd_end до используемой точности,
    только если истинный физический момент неотличим от границы суток —
    вероятность на практике неотличима от нуля, а координация между
    соседними вызовами `_find_phase` ради этого стоила бы архитектурной
    сложности, которую нечем оправдать. Если это когда-нибудь проявится —
    фаза отдастся с датой начала следующего месяца (round по 24:00:00,
    см. jd_to_utc) и будет корректно найдена ИМ, а не текущим: она не
    потеряется, а сдвинется на календарный день, которому по факту и
    принадлежит.
    """
    results, step = [], 0.5
        ...
        if prev is not None and prev * val < 0:
            ...
            for _ in range(48):  # достаточно итераций для точности ~1 сек
                ...
                if v > 0:
                    ...
                    lo = mid
            found = (lo + hi) / 2
            # Явный отсев, а не только докстринг: см. предупреждение выше.
            if found > jd_end:
                prev = val
                jd += step
                continue
        ...
        while jd < jd1:
            ...
            if cur_sign != prev_sign:
                ...
                events.append(CalendarEvent(
                    ...
                    description=f"{planet} входит {sign_into(cur_sign)}",
                ))
                prev_sign = cur_sign
