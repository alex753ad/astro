...
Модель оплаты: разовый платёж на 30 дней, автопродления нет. paid_until
считает activate_subscription (utcnow() + 30 дней, безусловно) — при апгрейде
остаток старого тарифа не переносится, отсчёт идёт с даты нового платежа.
Падение до free по истечении срока — задача tasks.expire_subscriptions.
