...
class NatalChart(Base):
    ...
    # `label` — из первого коммита, `name` добавлен позже (c9e20ac, 29.05.2026)
    # ровно под то же значение — пользовательское имя карты, дублирующее
    # BirthDataInput.name. Разведка 07.09.2026: у `label` нет ни одного
    # писателя и ни одного читателя во всём проекте, кроме одной строки в
    # /profile/export, которая экспортирует его как есть (то есть всегда
    # NULL). Он не второй источник истины — он ничей источник вовсе.
    # `name` заполняется при создании карты (main.py) и возвращается в API;
    # `label` оставлен как есть, не заполняется намеренно — не путать со
    # вторым местом для того же имени. Удаление колонки — отдельная задача
    # (миграция, решение владельца), сюда не входит.
...
# ── Слой 2: память Аристеи о пользователе между сессиями ──
class AstreaMemory(Base):
    """Компактная память Аристеи об одном человеке (одна строка на пользователя).

    summary — сводка до ~120 слов: кто человек, его цели, что он решил или
    отметил сделанным, что советовала Аристея. rag-chat подмешивает её в
