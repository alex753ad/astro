...
ADMIN_ROUTES = [
    ...
    # Аудит 23.08.2026, находка 2.5: ручка висела под префиксом /admin, но
    # пускала любого залогиненного — по коду ответа различались «нет такого
    # кода» / «истёк» / «исчерпан», то есть оракул для перебора промокодов.
    # Параметры в query, а не в теле — иначе FastAPI ответит 422 раньше, чем
    # отработает зависимость, и тест начнёт проходить впустую.
    ("post", "/api/v1/admin/coupons/apply?code=WHATEVER&plan=pro"),
]
