...
# Отображаемые названия тарифов для писем. Внутренние идентификаторы
# ('free'/'lite'/'pro'/'premium') в БД/проверках доступа не меняются.
TIER_NAMES = {"free": "Бесплатный", "lite": "Вега", "pro": "Лира", "premium": "Орион"}



def _plural(n: int, one: str, few: str, many: str) -> str:
    """Склонение существительного по числу: 1 месяц, 3 месяца, 12 месяцев."""
...
def _tier_limits(tier: str) -> dict:
    ...
    Импорт ленивый: rate_limits сам тянет отсюда TIER_NAMES (внутри функций),
    и модульный импорт в обратную сторону замкнул бы круг.

    Заводится потому, что письма обещали цифры, набранные руками, и обещания
    разошлись с сеткой: приветственное письмо Веги сулило транзиты на 12
    месяцев при transits_months = 1, а письмо Лиры — 5 PDF при pdf_per_month
    = 15. Первое человек читает сразу после оплаты.
    """
...
def build_broadcast_ai_prompt(period_label: str, transits: list[dict]) -> str:
    ...
    return (
        ...
        "ПРАВИЛА РАБОТЫ С ФАКТАМИ: транзиты ниже уже посчитаны точно — не вычисляй их сам "
        "и не добавляй других. Если список пуст — не выдумывай транзиты, пиши в общем тоне месяца. "
        "Заверши мягким приглашением на консультацию.\n\n"
        f"ТРАНЗИТЫ МЕСЯЦА:\n{transit_block}"
    )
...
async def send_lite_welcome(to: str, name: str | None = None) -> bool:
    ...
    # Горизонт транзитов — из TIER_FLAGS, а не цифрой в тексте: письмо
    # обещало 12 месяцев при transits_months = 1 (сетка от 19.08.2026).
    _lite_tr = _tier_limits("lite")["transits_months"]
    greeting = f"Привет, {name}! 🎉" if name else f"Добро пожаловать в {TIER_NAMES['lite']}! 🎉"
    body = (
        _h2(greeting)
        + _p(
        ...
        + f'<table width="100%" cellpadding="0" cellspacing="0" style="margin:0 0 20px;">'
          ...
          f'<tr><td style="padding:8px 0;border-bottom:1px solid #ece7f8;">'
          f'<span style="color:#9060C8;font-weight:700;">🔭</span>'
          f'<span style="color:#3d3060;font-size:15px;margin-left:10px;">Транзиты на {_lite_tr} {_plural(_lite_tr, "месяц", "месяца", "месяцев")} вперёд</span></td></tr>'
...
async def send_lite_day14(to: str, name: str | None = None) -> bool:
    ...
    body = (
        ...
        + _p(
            '<span style="font-size:12px;color:#a090c0;">'
            # Дословно как в окне оплаты (PaywallModal.jsx, поле price):
            # письмо и витрина обязаны говорить одно и то же. Прежняя строка
            # «Отмена в любой момент · Без обязательств» обещала механизм,
            # которого нет — автопродления в проекте нет, подписка разовая,
            # и отменять человеку нечего.
...
async def send_pro_welcome(to: str, name: str | None = None) -> bool:
    ...
    # Числа из TIER_FLAGS: письмо обещало 5 PDF при pdf_per_month = 15.
    _pro = _tier_limits("pro")
    _pro_pdf = _pro["pdf_per_month"]
    _pro_int = _pro["interpretations_per_month"]
...
# ── Пилот: прощание/спящий/eom ──
async def send_pilot_farewell(
    to: str,
    windows: list[str],
    ...
    offer_text — что даёт код (напр. "Pro на 3 месяца по сниженной цене").
    ...
    if windows:
        items = "".join(
            f'<li style="margin:0 0 10px;color:#3d3060;font-size:15px;line-height:1.6;">{w}</li>'
            for w in windows[:3]
        )
        windows_html = (
    ...
    if promo_code and checkout_url:
        deadline_line = f" Действует до {deadline}." if deadline else ""
        offer = offer_text or f"{TIER_NAMES['pro']} — 690 ₽ в месяц на 3 месяца"
        stay_html = (
            _p("Если этот месяц был полезен — есть способ остаться.")
            + _p(f"Промокод для участников теста: <strong>{promo_code}</strong>."
                 f" {offer}.{deadline_line}")
            + _btn("Остаться", checkout_url)
        )
