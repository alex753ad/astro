...
# `transit_interp:` (main.py). Кэш живёт 30 дней; поднимать при правке,
# которая должна заменить уже закэшированные тексты. Обзор периода версии не
# требует: он идёт через `custom_prompt`, и ключ меняется с хешем промпта сам.
# 2 — переход на «ты», 24.09.2026.
TRANSIT_PROMPT_VERSION = 2
...
TRANSIT_EVENT_PROMPT = """Тебя зовут Аристея, ты — навигатор решений, а не предсказатель.
...
## Тон
Ты — Аристея: спокойная, конкретная, без фатализма. Живой язык, без астро-пафоса и клише.
""" + ADDRESS_RULE + """
...
TRANSIT_TEMPLATES = {
    ...
    ("Jupiter", "conjunction"): (
        "Транзит Юпитера в соединении с твоим натальным {natal_planet} — "
        "период расширения и возможностей в сфере, связанной с {natal_planet}. "
        "Это время оптимизма, роста и удачных совпадений. Сейчас стоит открываться новому."
    ),
    ("Jupiter", "opposition"): (
        "Транзит Юпитера в оппозиции к натальному {natal_planet} — "
        "момент переоценки и поиска баланса. Возможно чрезмерное расширение; "
        "важно не брать на себя больше, чем можешь осилить."
    ),
    ("Jupiter", "square"): (
    ...
    ("Jupiter", "sextile"): (
        ...
        "возможности появляются, но требуют твоей инициативы. "
    ...
    ("Neptune", "conjunction"): (
        ...
        "интуиция, но и риск иллюзий. Доверяй, но проверяй."
    ),
    ("Neptune", "opposition"): (
    ...
    ("Pluto", "square"): (
        ...
        "внутренняя или внешняя. Отпусти то, что больше не служит тебе."
    ),
    ...
    ("Mars", "square"): (
        ...
        "кратковременное напряжение и раздражение. Направь энергию "
...
def get_template_transit_text(
    ...
    return (
        ...
        f"Обрати внимание на темы, связанные с {natal_planet} в твоей карте."
    )
