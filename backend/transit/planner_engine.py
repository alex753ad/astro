...
def is_moon_week_locked(
    ...
    Правило целиком:
      ...
      * **вперёд — по тарифу**, `planner_weeks_ahead` недель, СЧИТАЯ текущую.
        У free это 1 — текущая неделя и не дальше. У платных `None`, то есть
        всё окно: замков на проходах Луны у них нет вовсе (решение владельца
        16.09.2026). Раньше там стояла четвёрка, и платный видел закрытые
        недели, купить которые было нечем, — состояние без выхода.
    ...
    # None — «всё окно», общее правило безлимита в TIER_FLAGS. Проверять
    # границы недели незачем: закрытых проходов у такого тарифа нет.
    if weeks is None:
        return False
