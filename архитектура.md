# Astrea Timeline — Архитектура проекта (v2.3)
...
## 15. Снимок репозитория (root) — текущее состояние (2026-07-09)

Файлы и директории, найденные в корне репозитория:

- .dockerignore, .gitignore, .env.example, .vscode/
- .github/
- ARCHITECTURE_v2.md (дубликат/альтернативная версия документа)
...
- alembic/, alembic.ini
- различные утилиты и тесты: main.py, engine.py, house_passages.py, generate_pdf.py, natal_pdf.py
- большие артефакты/данные: log.json, natal_chart.pdf
- вспомогательные текстовые файлы: заметки и шаблоны (замечания.txt, образец.txt и пр.)

Примечание: в корне также есть файл ARCHITECTURE_v2.md (размер совпадает с этим документом). Рекомендую держать одну canonical-версию (например, ARCHITECTURE_v2.md или архитектура.md) и удалить/перенаправить другой файл, чтобы избежать рассинхронизации.
