...
#
# ⚠️ Сама по себе эта цель НЕ поднимает версию пакета, у которого уже есть пин
# в локе: uv берёт существующий лок как предпочтение и оставляет старую версию,
# если она всё ещё проходит по новому диапазону. То есть расширение границы в
# pyproject (например `<5.0.0` → `<6.0.0`) + `make lock` даёт нулевой diff, и
# в образ по-прежнему едет старая версия. Поднимать адресно:
#   uv pip compile ... --upgrade-package <имя>
# Проверено 14.09.2026 на bcrypt: без флага лок остался на 4.3.0.
lock:
