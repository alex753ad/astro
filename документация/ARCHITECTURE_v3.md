...
> ⛔ **УСТАРЕЛО (отмечено 23.08.2026). Не использовать как справку — читать
> `ARCHITECTURE V4.md` рядом, а по тарифам, платежам и деплою — `CLAUDE.md`
> в корне (он источник истины).**
>
> Что здесь неверно фактически:
> - раздел `payments/` описывает Robokassa и Stripe — оба удалены 19.08.2026
>   как мёртвый код, сейчас единственный провайдер ЮKassa;
> - переменные `ROBOKASSA_*` / `STRIPE_*` в списке env не существуют;
> - цены и раскладка возможностей по тарифам не соответствуют сетке,
>   утверждённой владельцем 19.08.2026;
> - деплой описан как Vercel (фронтенд) + Railway (бэкенд) — всё давно на
>   одном Timeweb VPS под Docker Compose, Railway cron не используется.
>
> Файл оставлен как исторический срез миграции v2 → v3, а не как документация.

Документ описывает архитектуру системы после масштабной миграции (август 2026)
**по состоянию на момент написания**. Источник истины — код в репозитории.
